...
using UnityEngine;
using UnityEngine.UI; // Only allow this script to be attached to the object with the healthbar slider:
[RequireComponent(typeof(Slider))]
public class Healthbar : MonoBehaviour {

    // Visible health bar ui:
    private Slider _healthbarDisplay;
    ...
    [Tooltip("Minimum possible heath: (default is 0)")] public float minimumHealth;
    ...
    private void Start()
    {
        ...
        {
            _healthbarDisplay = GetComponent<Slider>();
        }

        // Set the minimum and maximum health on the healthbar to be equal to the 'minimumHealth' and 'maximumHealth' variables:
        _healthbarDisplay.minValue = minimumHealth;
        _healthbarDisplay.maxValue = maximumHealth;
    ...
    private void Update()
    {
        _healthPercentage = int.Parse((Mathf.Round(maximumHealth * (health / 100f))).ToString(CultureInfo.CurrentCulture));
    ...
    private void UpdateHealth()
    {
        ...
        if (_healthPercentage <= lowHealth && health >= minimumHealth && transform.Find("Bar").GetComponent<Image>().color != lowHealthColor)
        {
            ChangeHealthbarColor(lowHealthColor);
        }
        else if (_healthPercentage <= highHealth && health > lowHealth)
        {
            var lerpedColorValue = (float.Parse(_healthPercentage.ToString()) - 25) / 41;
            ChangeHealthbarColor(Color.Lerp(lowHealthColor, mediumHealthColor, lerpedColorValue));
        }
        else if (_healthPercentage > highHealth && health <= maximumHealth)
        {
            var lerpedColorValue = (float.Parse(_healthPercentage.ToString()) - 67) / 33;
            ChangeHealthbarColor(Color.Lerp(mediumHealthColor, highHealthColor, lerpedColorValue));
        }

        _healthbarDisplay.value = health;
    }
    ...
    private void ChangeHealthbarColor(Color colorToChangeTo)
    {
